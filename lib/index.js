...
d3.csv("./data.csv").then(function (data) {
  ...
  svg.append("text").attr("x", width / 2).attr("y", 0 - (margin.top - 30)).attr("text-anchor", "middle").style("font-size", "18px").style("padding-bottom", "10px").text("Total Sanctions by Country"); // X axis
  ...
  var colors = d3.scaleLinear().domain([0, data.length - 1]).range(palettes[Math.floor(Math.random() * palettes.length)]); // Add Y axis
  ...
  .attr("class", "y axis").selectAll("text").attr('font-weight', 'bold');
  var bars = svg.selectAll("bar").data(data).enter().append("g").attr("class", "barWithLabel"); // Bars

  bars.append("text").attr("x", function (d) {
    return x(d.Country) + x.bandwidth() / 2 - d.Value.toString().length * 3;
  }).attr("y", function (d) {
    return y(d.Value) - 5;
  }).attr("class", "textLabel").style("font-size", "12px").style("visibility", "hidden").text(function (d) {
    return d.Value;
  });
